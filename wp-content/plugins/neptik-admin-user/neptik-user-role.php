<?php
/*
Plugin Name: Neptik Admin User Control
...
Version: 1.0.0
Author: Paul Hayler
Author URI: http://neptik.com
Author Email: paul.hayler@neptik.com
License:
...
if ( !class_exists( 'TD_WebmasterUserRole' ) ) {
	 ...
	 *--------------------------------------------*/
		...
		const name = 'Neptik Admin User';

		const slug = 'neptik-admin-user-role';
	 ...
	 *--------------------------------------------*/
		...
		function __construct() {
			...
			load_plugin_textdomain( 'neptik-admin-user-role', false, dirname( plugin_basename( __FILE__ ) ) . '/lang' );
			...
			$site_version = get_site_option( 'neptik-admin-user-role-version' );
			if( $site_version!=self::version ) {
				$this->deactivate( false );
				$this->activate( false );
				update_site_option( 'neptik-admin-user-role-version', self::version );
			}
...
if ( !isset( $neptik_admin_user_role ) ) $neptik_admin_user_role = new TD_WebmasterUserRole();
register_activation_hook( __FILE__, array( $neptik_admin_user_role, 'activate' ) );
register_deactivation_hook( __FILE__, array( $neptik_admin_user_role, 'deactivate' ) );
